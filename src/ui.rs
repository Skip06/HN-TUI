...
pub fn render(frame: &mut Frame, app: &mut App) {
    // this is what gets callwed every frame
    let area = frame.area();
...
fn draw_stories(frame: &mut Frame, app: &mut App) {
    ...
    let items: Vec<ListItem> = app
        ...
        .map(|(i, story)| {
            ...
            let meta_line = Line::from(vec![Span::raw(format!(
                "     {} pts · {} · {}",
                story.score.unwrap_or(0),
                story.by.as_deref().unwrap(),
                time_ago(story.time.unwrap())
            ))]);
    ...
    let header = Paragraph::new(format!(
        "Read {:?} HackerNews   .   q: Quit  o: Open  Enter: Comments  up/down: Navigate",
        app.feed
    ))
    .style(Style::default().fg(Color::LightYellow));

    frame.render_widget(
        header.block(Block::new().fg(Color::Red).borders(Borders::ALL)),
        chunks[0],
    );
